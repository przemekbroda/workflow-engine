    ...
    where TEvent : class
{
    ...
    Task<TEvent> ExecuteAsync(TEvent @event, TState state, CancellationToken cancellationToken);
    TState TryUpdateState(TEvent @event, TState state);
    ...
    Task AfterExecutionAndStateUpdate(TEvent @event, TState state, CancellationToken cancellationToken);
}